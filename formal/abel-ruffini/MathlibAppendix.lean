/-!
Exact excerpts from `Mathlib.FieldTheory.AbelRuffini` in mathlib v4.31.0.

This file is a presentation source for the article's side-by-side appendix. The declarations remain
in their original mathlib module; these excerpts are not compiled as part of the companion library.
-/

-- region appendix-tower
...
-- endregion appendix-tower

-- region appendix-solvable-by-rad
variable (F E) in
...
protected theorem solvableByRad.induction (motive : ∀ x, x ∈ solvableByRad F E → Prop)
  ...
  exact h
-- endregion appendix-solvable-by-rad

-- region appendix-radical-step
...
-- endregion appendix-radical-step

-- region appendix-main-induction
...
-- endregion appendix-main-induction

-- region appendix-normalization
...
theorem isSolvable_gal_of_irreducible {x : E} (hx : x ∈ solvableByRad F E) {q : F[X]}
  ...
  aesop
-- endregion appendix-normalization
